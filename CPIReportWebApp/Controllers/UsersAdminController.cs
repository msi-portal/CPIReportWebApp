...
namespace CPIReportWebApp.Controllers
{
    ...
    public class UsersAdminController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Email,Id,SupplierCode,RolesList")] EditUserViewModel editUser, params string[] selectedRole)
        {
                ...
                if (postedFile != null)
                {
                    ...
                    foreach (string row in csvData.Split(new string[] { "\r\n" }, StringSplitOptions.None))
                    {
                        if (!string.IsNullOrEmpty(row))
                        {
                            ...
                            if (rowVar[0] != "suppliercode")
                            {
                                var user = new ApplicationUser();
                                user.UserName = rowVar[1];
                                user.Email = rowVar[2];
